import { defineCustomEventTarget, EventTarget } from "event-target-shim"
...
export default class RTCPeerConnection extends EventTarget<RTCPeerConnectionEventMap> {
  ...
  async setRemoteDescription(
    ...
    const { newTransceivers, transceiversInfo } =
  ...
  _registerEvents(): void {
    ...
    addListener(this, "peerConnectionSignalingStateChanged", (ev: any) => {
      ...
      if (remoteSdp.type && remoteSdp.sdp) {
        switch (this.signalingState) {
          case "stable":
            this._currentRemoteDescription = new RTCSessionDescription(
              remoteSdp
            )
            this._pendingRemoteDescription = null
            break
          ...
          case "have-remote-pranswer":
            this._currentRemoteDescription = null
            this._pendingRemoteDescription = new RTCSessionDescription(
              remoteSdp
            )
            break
        }
